...
const Checkout = ({ resetCart, orderRedux }) => {
  ...
  const { user, cart, sale, vouchers, vouchersPremium, point } = useSelector(
    (state) => ({
      user: state.User,
      cart: state.Cart,
      sale: state.Event,
  ...
  const total = cart.reduce(
    (total, item) =>
      total + (item.price - (item.price * sale) / 100) * item.count,
    0
  );
  ...
  const handlePayment = () => {
    ...
    order({
      variables: {
        data: {
          price: parseInt(
            total - (total * couponRate) / 100 - couponRatePre,
            10
          ),
          namePro,
          status: "Chờ xử lý",
                                      ...
                                      <label>
                                        ...
                                        <span>
                                          ...
                                          <div className='discount-item__name'>
                                            Giảm {voucher.disCount}% giá trị đơn
                                            hàng
                                          </div>
                                          <div className='discount-item__condition'>
                                            Đơn tối thiểu{" "}
                                            {currencyFormat(voucher.condition)}đ
                                          </div>
                                        </span>
                                      </label>
                                    </div>
                          ...
                          <div className='modal-footer'>
                            <button
                              onClick={() => {
                                if (
                                  coupon > 0 &&
                                  total < vouchers[coupon].condition
                                ) {
                                  setCouponRate(0);
                                }
                                const index = coupon;
                  ...
                  <div className='cart-detail cart-total bg-light p-3 p-md-4'>
                    ...
                    <p className='d-flex'>
                      ...
                      <span>
                        ...
                        {currencyFormat(
                          parseInt(
                            (total * couponRate) / 100 + couponRatePre,
                            10
                          )
                        )}
                      </span>
